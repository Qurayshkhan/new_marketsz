...
The filter functionality allows administrators to filter packages by various criteria including status, date range, sender, tracking ID, and total value range. The implementation includes both backend (Laravel) and frontend (Vue.js) components.
...
| `sender_id`       | Dropdown | Customer/sender ID         | `sender_id=3`          |
...
GET /admin/packages?sender_id=3&status=2
...
GET /admin/packages?status=1&date_from=2025-01-01&date_to=2025-01-31&sender_id=3&tracking_id=TRACK&total_value_min=100&total_value_max=500
```
...
-   Returns customers list for sender dropdown
